...
public class HomcMain extends Activity {
...
//        });
        ...
        final Switch led = (Switch) findViewById(R.id.LED);
        led.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                if (isChecked) {
                    sendMessage("/to/LEDswitch", "{\"LED1\":\"on\"}");
                } else {
                    sendMessage("/to/LEDswitch", "{\"LED1\":\"off\"}");
                }
            }
        });

        final Switch relais = (Switch) findViewById(R.id.relais);
        relais.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                if (isChecked) {
                    sendMessage("/to/switch", "{\"value\":\"on\"}");
                } else {
                    sendMessage("/to/switch", "{\"value\":\"off\"}");
                }
            }
        });
    }
    ...
    private void subscribe() {
        Bundle data = new Bundle();
        String[] topics = {"/arduino/counter", "/from/status", "/raspberry/healthstatus", "/from/temperature", "/from/humidity"};
